...
class RecordFormViewController: UIViewController {
    ...
    func setupUI() {
        setNavigationTitle(title: "Add an observation")
    ...
    func setupDropDown() {
        ...
        nameDropDown.selectionAction = { [weak self] (index: Int, item: String) in
            ...
            self.viewModel.recordModel.name = item
            self.viewModel.recordModel.plantID = self.viewModel.plants[index].id
        }
    }
    ...
    func subscribe() {
        viewModel.$recordModel
            ...
            .sink { [weak self] recordModel in
                guard let self = self else { return }
                saveButton.isEnabled = (recordModel.date != nil && recordModel.info.checkValidation() && recordModel.name.checkValidation())
                if self.viewModel.isEditing {
                    self.namePlantButton.setTitle(recordModel.name, for: .normal)
                    self.descriptionTextField.text = recordModel.info
                    self.dateTextField.text = recordModel.date?.toString()
                }
            }
    ...
    @objc func dateChanged() {
        viewModel.recordModel.date = datePickerView.date
...
extension RecordFormViewController: UITextFieldDelegate {
    func textFieldDidChangeSelection(_ textField: UITextField) {
        if textField == descriptionTextField {
            viewModel.recordModel.info = textField.text
        }
    }
